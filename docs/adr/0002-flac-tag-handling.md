...
* User-authored metadata (such as primary artist names or custom release IDs) must never be erased or overwritten with generic fallbacks.
...
* **Track & Album Dynamic Range (`DYNAMIC RANGE`, `ALBUM DYNAMIC RANGE`)**:
  * Computed via EBU R 128 / `drmeter`. Track DR is written to `DYNAMIC RANGE`.
  * Album DR is the rounded arithmetic mean of all track DR scores in the album, written to `ALBUM DYNAMIC RANGE`.
* **AcoustID Fingerprints (`ACOUSTID FINGERPRINT`)**:
  * Computed via `fpcalc` (Chromaprint). Written once and skipped if already present.
